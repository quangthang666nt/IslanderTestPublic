...
namespace Rewired.UI.ControlMapper
{
	...
	public class ControlMapper : MonoBehaviour
	{
		...
		public void OnInputFieldActivated(InputFieldInfo fieldInfo)
		{
			...
			if (LocalizationManager.GetTranslation("Input Actions/" + action.descriptiveName) != null)
			{
				text = LocalizationManager.GetTranslation("Input Actions/" + action.descriptiveName);
			}
			if (action.type == InputActionType.Axis)
			{
				if (fieldInfo.axisRange == AxisRange.Positive)
				{
					text = LocalizationManager.GetTranslation("Input Actions/" + action.positiveDescriptiveName);
				}
				if (fieldInfo.axisRange == AxisRange.Negative)
				{
					text = LocalizationManager.GetTranslation("Input Actions/" + action.negativeDescriptiveName);
				}
			}
			string actionName;
			if (text.Length > 0)
			{
				...
				else
				{
					...
					{
						actionName = ((!string.IsNullOrEmpty(action.positiveDescriptiveName)) ? text : (text + " +"));
					}
					else
					{
						if (fieldInfo.axisRange != AxisRange.Negative)
						...
						actionName = ((!string.IsNullOrEmpty(action.negativeDescriptiveName)) ? text : (text + " -"));
					}
				}
			}
			...
			for (int i = 0; i < _mappingSets.Length; i++)
			{
				...
				if (mappingSet.actionListMode == MappingSet.ActionListMode.ActionCategory)
				{
					...
					for (int j = 0; j < actionCategoryIds.Count; j++)
					{
						...
						{
							string text = null;
							if (LocalizationManager.GetTranslation("Input Actions/" + item.descriptiveName) != null)
							{
								text = LocalizationManager.GetTranslation("Input Actions/" + item.descriptiveName);
							}
							string text2 = null;
							if (LocalizationManager.GetTranslation("Input Actions/" + item.positiveDescriptiveName) != null)
							{
								text2 = LocalizationManager.GetTranslation("Input Actions/" + item.positiveDescriptiveName);
							}
							string text3 = null;
							if (LocalizationManager.GetTranslation("Input Actions/" + item.negativeDescriptiveName) != null)
							{
								text3 = LocalizationManager.GetTranslation("Input Actions/" + item.negativeDescriptiveName);
							}
							if (item.type == InputActionType.Axis)
							{
								if (_showFullAxisInputFields)
								{
									string labelText = item.descriptiveName;
									if (text != null)
									{
										labelText = text;
									}
								...
								if (_showSplitAxisInputFields)
								{
									string labelText2 = ((!string.IsNullOrEmpty(item.positiveDescriptiveName)) ? item.positiveDescriptiveName : (item.descriptiveName + " +"));
									if (text2 != null)
									{
										labelText2 = text2;
									}
									GUILabel gUILabel2 = CreateLabel(labelText2, inputGridActionColumn, new Vector2(0f, num));
									gUILabel2.rectTransform.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, _inputRowHeight);
									inputGrid.AddActionLabel(mappingSet.mapCategoryId, item.id, AxisRange.Positive, gUILabel2);
									num -= _inputRowHeight;
									string labelText3 = ((!string.IsNullOrEmpty(item.negativeDescriptiveName)) ? item.negativeDescriptiveName : (item.descriptiveName + " -"));
									if (text3 != null)
									{
										labelText3 = text3;
									}
									gUILabel2 = CreateLabel(labelText3, inputGridActionColumn, new Vector2(0f, num));
									gUILabel2.rectTransform.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, _inputRowHeight);
							...
							{
								string labelText4 = item.descriptiveName;
								if (text != null)
								{
									labelText4 = text;
								}
								GUILabel gUILabel2 = CreateLabel(labelText4, inputGridActionColumn, new Vector2(0f, num));
								gUILabel2.rectTransform.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, _inputRowHeight);
								inputGrid.AddActionLabel(mappingSet.mapCategoryId, item.id, AxisRange.Positive, gUILabel2);
								num -= _inputRowHeight;
							}
		...
		private void PopulateInputFieldGroup(InputActionSet actionSet, ControllerMap controllerMap, ControllerType controllerType, int controllerId, int maxFields)
		{
			...
			{
				string text = null;
				string term = "Settings/Controls/Mapper/'" + item.elementIdentifierName + "'";
				if (LocalizationManager.GetTranslation(term) != null)
				{
					text = LocalizationManager.GetTranslation(term);
				}
				string label = item.elementIdentifierName;
				if (text != null)
				{
					label = text;
				}
				if (item.elementType == ControllerElementType.Button)
				{
					...
					inputGrid.PopulateField(currentMapCategoryId, actionSet.actionId, actionSet.axisRange, controllerType, controllerId, num, item.id, label, invert: false);
				}
				else if (item.elementType == ControllerElementType.Axis)
				{
					if (actionSet.axisRange == AxisRange.Full)
					{
						...
						inputGrid.PopulateField(currentMapCategoryId, actionSet.actionId, actionSet.axisRange, controllerType, controllerId, num, item.id, label, item.invert);
					}
